...
// EmailForm moved outside main component for performance
function EmailForm({
  email,
  setEmail,
  termsAgreed,
...
export default function SwapNftsScreen() {
  ...
  // Only preload first 6 NFT images
  const preloadImages = useCallback((nfts: NFT[]) => {
    nfts.slice(0, 6).forEach((nft) => {
      if (nft.image) {
  ...
  useEffect(() => {
    ...
    fetchNFTs();
  }, [isConnected, address, preloadImages]);
      ...
      if (result.success) {
        ...
        if (response.ok) {
          ...
          if (
            ...
            toast.error(
              `Failed to save swap record: ${(errorData as { error: string }).error}`
            );
          } else {
  ...
  const handleViewMore = () => {
    setDisplayedNfts((prev) => {
      const currentlyDisplayed = prev.length;
                    ...
                    <div className="relative h-[70%] border-b-2 border-black">
                      {nft.image ? (
                        <Image
                          src={nft.image}
                          alt={
                            nft.name ? `NFT image for ${nft.name}` : "NFT image"
                          }
                          fill
                          className="object-cover"
                          priority={displayedNfts.indexOf(nft) < 6}
                          loading={
